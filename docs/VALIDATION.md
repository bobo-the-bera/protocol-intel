...
The owner then authorized scheduled monitoring and daily digests. The implementation adds 15-minute due-source cycles, HIGH/CRITICAL alerts, and a previous-UTC-day digest due at 08:00 UTC with zero digest inference. Stored cursor, outbox and cycle records support catch-up, retry and failure visibility. Unit/Postgres tests cover day boundaries, late completion, exact focus deduplication, baseline exclusion, upload failure across midnight, missing report evidence, concurrent workers, definite/ambiguous delivery and continued digest work after collection failure. CI and the first live scheduled cycle/digest require recording after execution; no release criterion is waived by this pilot authorization.
...
The run failed evidence-ID validation and sent no analysis report to Telegram. [Receipt run 34469790454](https://github.com/bobo-the-bera/protocol-intel/actions/runs/34469790454) succeeded on 10 September: all references in the five findings are exact supplied snapshot IDs, embedded in explanatory evidence sentences rather than standalone IDs. The recovery path extracts only exact bracketed legacy IDs, rejects unknown/malformed references, preserves original prose, and makes no model call. New preview schemas constrain citations to the allowed IDs. Live recovery and end-to-end Telegram report delivery are still pending; inspection alone did not complete them.

The saved response is a baseline sample of 8/69 available pages, with truncated excerpts and no before/after comparison. Its authorization concern explicitly depends on an incomplete API excerpt and does not establish a deployed vulnerability. The report now retains the model's sample/context notes even when findings exist. These broad baseline risk observations are not evidence of newly detected changes.

The receipt measured **2.864 MiB across 163 application archive objects**: NEAR **2.668 MiB**, HyperLend **0.196 MiB**, no shared bytes. The whole Postgres database occupied **9.398 MiB**. Both source inventories remained fully baselined (NEAR 70/70, HyperLend 66/66). These are current sizes, not monthly growth or provider invoices. Receipt inspection made no model call and sent no Telegram message. Its call-cost scenarios describe the original Sol workload, not a measured result from the new Luna screening defaults.
